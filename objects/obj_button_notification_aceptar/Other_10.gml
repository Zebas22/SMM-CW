...
{
	global.directorio_temp = getDire1("SMM-CW")
...
var day = formatNumber(current_day);
var month = formatNumber(current_month);
var year = formatNumber(current_year);
var date = day + "/" + month + "/" + year;

var hour = formatNumber(current_hour);
var minute = formatNumber(current_minute);
...
ds_map_add(_map2, "water_level", (room_height - 16))
ds_map_add(_map2, "water_level_limit", (room_height - 16))
...
ds_map_add_map(mundo, "INICIO", _wrapper)
...
_string_code += _hash
SaveStringToFile(((global.directorio + obj_input.new_texto) + ".cw"), _string_code)
global.save_changes = (obj_input.new_texto + ".cw")
global.last_level = (obj_input.new_texto + ".cw")
global.levelguardabot = global.last_level
ds_map_destroy(_wrapper)
with (obj_load_image)
    instance_destroy()
scr_toast(0, "Nivel Guardado Corectamente")
