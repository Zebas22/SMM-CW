...
if (global.nm_play != "")
{
	var _str = ("token=" + global.token + "&discord_id=" + hash_encode(global.my2) + "&auth_code=" + global.my5 + "&id=" + hash_encode(global.level_id) + "&stats="+ hash_encode("intentos"))
	...
	if(ds_map_exists(mundos, "HOME"))
			_wrapper = ds_map_find_value(mundos, "HOME")
		else
	...
	if (global.bg_level == "mountain" && global.apariencia == 3 && global.modo_noche == 1 && !instance_exists(obj_firemaker))
        ...
        {
            case "airship":
               if (global.apariencia == 3) && (global.modo_noche == 1)
			   {
                    layer_hspeed(1, -0.1)
                    layer_hspeed(2, -0.2)
                    layer_hspeed(3, -0.3)
